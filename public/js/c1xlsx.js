...
function xlsx(file) { 
	...
	var result, zip = new JSZip(), zipTime, processTime, s, content, f, i, j, k, l, t, w, sharedStrings, styles, index, data, val, formula /* GrapeCity: Add formula variable.*/, style, borders, border, borderIndex, fonts, font, fontIndex,
		docProps, xl, xlWorksheets, worksheet, contentTypes = [[], []], props = [], xlRels = [], worksheets = [], id, columns, cols, colWidth, cell, row, merges, merged, rowStr, rowHeightSetting, groupLevelSetting, firstCell, rowVisible, hiddenColumns, idx, colIndex, groupLevel, frozenPane, frozenRows, frozenCols, fills, fill, fillIndex, /* GrapeCity: Add fill color setting related variables.*/
		...
		colsWidth, rowsHeight, height, /* GrapeCity: Stores the column width and row height for importing.*/
		mergeCellArray, mergeRange, mergeCells, /* GrapeCity: Stores merge cell range for importing.*/
	...
	function getColor(s) {
		...
		if (s.search(/fgcolor/i) === -1) {
			return undefined;
		}
		s = s.substring(s.indexOf('<fgColor'), s.indexOf('/>'));
		if (s.indexOf('rgb=') !== -1) {
			colorType = 'rgbColor';
			theme = undefined;
			value = getAttr(s, 'rgb');
   ...
   if (typeof file === 'string') { // Load
		...
		if (s = zip.file('xl/styles.xml')) { // Get style info from "xl/styles.xml"
			...
			while (--i) {
				...
				styles.unshift({
					formatCode: f,
					type: t,
					bold: font ? font.bold : undefined,
					italic: font ? font.italic : undefined,
					underline: font ? font.underline : undefined,
					fontSize: font ? font.size : undefined,
					fontName: font ? font.name : undefined,
					fontColor: font ? font.color : undefined,
					fillColor: fill,
					hAlign: s[i].indexOf('<alignment') > -1 ? getAttr(s[i], 'horizontal') : 'left'
				});
				// GrapeCity End
			}
		}
		...
		while (i--) {
			...
			if (s.length > 0 && s[0].indexOf('<cols>') > -1) {
				...
				for (idx = cols.length - 1; idx > 0; idx--) {
					...
					for (colIndex = +getAttr(cols[idx], 'min') - 1; colIndex < +getAttr(cols[idx], 'max') ; colIndex++) {
						if (getAttr(cols[idx], 'hidden') === '1' && hiddenColumns.indexOf(colIndex) === -1) {
							hiddenColumns.push(colIndex);
						}
						colsWidth[colIndex] = colWidth;
					}
				}
			}
			w.colsWidth = colsWidth;
			w.rowsHeight = rowsHeight;
			// GrapeCity End
			...
			while (--j) { // Don't process j === 0, because s[0] is the text before the first row element
				...
				if (getAttr(s[j], 'customHeight') === '1') {
					...
					rowsHeight[+getAttr(s[j], 'r') - 1] = height;
				}
				// GrapeCity End
				...
				while (--k) { // Don't process l === 0, because k[0] is the text before the first c (cell) element
					...
					if (f.bold || f.italic || f.underline || f.fontName || f.fontSize || f.fontColor || f.fillColor || f.hAlign) {
						style = {
							bold: f.bold,
							italic: f.italic,
			...
			while (++i < l) {
				...
				while (++j < k) {
					...
					style = { // supported styles: borders, hAlign, formatCode and font style
						...
						color: cell.color, /* GrapeCity: Add color property*/
						fillColor: cell.fillColor, /* GrapeCity: Add fill color property*/
						underline: cell.underline /* GrapeCity: Add underline property*/
					};
					colWidth = 0;
		...
		while (--i) { 
			...
			if (style.bold || style.italic || style.fontSize || style.fontName || style.color) {
				...
				font.push('<color ', style.color ? 'rgb="FF' + style.color + '"' : 'theme="1"', '/>');
				// GrapeCity End
			...
			if (style.fillColor) {
				...
				fill.push('<fgColor rgb="FF', style.fillColor, '"/>');
